  ...
  return (
    <footer className="grain relative overflow-hidden border-t border-white/10 bg-void">
      ...
      <Shell className="relative">
        ...
        <div className="grid gap-12 py-16 lg:grid-cols-[1.15fr_2fr] lg:gap-20 lg:py-20">
          <div>
            <Link href="/" aria-label="SnZ Ventures — home" className="group inline-flex items-center gap-3">
              ...
              />
              <span className="font-display text-[1.25rem] tracking-[-0.02em] text-paper">
                SnZ Ventures
              </span>
            </Link>

            <p className="mt-7 max-w-sm font-display text-[1.45rem] leading-[1.2] tracking-[-0.018em] text-navy-100">
              Geography should not be a barrier to ambition.
            </p>

            <ul className="mt-7 flex flex-wrap gap-2">
              {company.attributes.map((a) => (
                <li key={a} className="label border border-white/12 px-2.5 py-1.5 text-navy-300">
                  {a}
                </li>
              ))}
            </ul>
                ...
                <ul className="mt-5 space-y-3">
                  {group.links.map((l) => (
                    <li key={l.href}>
                      <Link
                        href={l.href}
                        className="text-[0.87rem] text-navy-200 transition-colors duration-300 hover:text-paper"
                      >
                        {l.label}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col gap-6 border-t border-white/10 py-8 md:flex-row md:items-center md:justify-between">
          ...
          {company.social.linkedin && (
            <a
              ...
              className="flex h-10 w-10 items-center justify-center border border-white/15 text-navy-200 transition-colors hover:border-moss-400/60 hover:text-moss-300"
            >
        ...
        {/* Legal */}
        <div className="flex flex-col gap-4 border-t border-white/10 py-7 text-[0.76rem] leading-relaxed text-navy-400 md:flex-row md:justify-between">
          <p>
        ...
        <div className="border-t border-white/10 py-6 text-[0.72rem] leading-relaxed text-navy-500">
          <p>
            ...
            <Link
              ...
              className="underline underline-offset-2 transition-colors hover:text-navy-300"
            >
              Image credits
            </Link>
            .
